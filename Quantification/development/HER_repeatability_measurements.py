...
data_directory = Path(r"C:\Users\AnnaWiniwarter\Dropbox (Spectro Inlets)\Development\Data\Quantification Application Note\HER on Pt repeatability")
PARENT_DIR = Path(r"C:\Users\AnnaWiniwarter\Dropbox (Spectro Inlets)\Development\Data\Quantification Application Note")
...
file_dict_HER = {"HERonPt_RnD1_oldMS": {"DIR": r"HER on Pt repeatability\2022-03-16 10_41_29 HER_on_Pt_repeatability_test_1",
              ...
              "HERonPt_RnD1_newMS": {"DIR": r"HER on Pt repeatability\2022-04-06 14_48_52 HER on Pt repeatability",
                                    ...
                                    "cp_tspan": [1639,4916],
                                    "tspan_list": [[2699, 2803], [3276, 3399], [3872, 4006], [4475,4605]], #[2157, 2197],
                                    "tspan_bg": [2272, 2482],},
...
def main():
    ...
    if WHICH_PART == "plot_CVs": # plot the CV part
        ...
        cvs_ec_only = ECMeasurement.read(data_directory / biologic_filename_cv, reader="biologic")
        ...
        HER_cal= cps.ecms_calibration_curve("H2", "M2", -2, tspan_list=cp_tspan_list, tspan_bg=cp_bg, ax="new", axes_measurement=axes_c, return_axes=True)
        print("HER calibration factor F= {:.3f}".format(HER_cal[0].F))
        HER_cal[1].annotate("F = {:.3f}".format(HER_cal[0].F), (0.2,0.8), xycoords="subfigure fraction")
        ...
        OER_cal= cps.ecms_calibration_curve("O2", "M32", 4, tspan_list=cp_tspan_list, tspan_bg=cp_bg, ax="new", axes_measurement=axes_c, return_axes=True)
        print("OER calibration factor F= {:.3f}".format(OER_cal[0].F))
        OER_cal[1].annotate("F = {:.3f}".format(OER_cal[0].F), (0.2,0.8), xycoords="subfigure fraction")
